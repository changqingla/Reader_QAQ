...
class ContextInjector:
    ...
    def inject_for_simple_interaction(self, session_id: str) -> List[Message]:
        ...
        # 安全检查：如果超过预期，记录警告（理论上不应该发生）
        expected_max = int(get_settings().compression_threshold_tokens * 0.4)  # 预期最大约40K
        if total_tokens > expected_max:
            logger.warning(
                f"⚠️  Simple interaction context larger than expected: "
                f"{total_tokens} tokens (expected < {expected_max}). "
                f"This may indicate compression is not working properly."
            )
